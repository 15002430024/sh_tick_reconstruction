...
- 11:30 - 13:00  午间休市      ❌ 剔除
- 13:00 - 14:57  连续竞价(下午) ✅ 保留
- 14:57 - 15:00  收盘集合竞价  ❌ 剔除

🔴 重要更新 (2018年8月20日起)：
- 上交所引入收盘集合竞价 (14:57-15:00)，与深交所一致
- 连续竞价下午时段改为 13:00-14:57
...
# 下午连续竞价时段
# ⚠️ 修正：2018年后上交所有收盘集合竞价 (14:57-15:00)，应排除
AFTERNOON_START = 130000000  # 13:00:00.000
AFTERNOON_END = 145700000    # 14:57:00.000 (不含，14:57后为收盘集合竞价)
...
def is_continuous_trading_time(tick_time: int) -> bool:
    ...
    - 下午：13:00:00.000 <= tick_time < 14:57:00.000
    ...
    Note:
        ⚠️ 2018年8月20日起上交所引入收盘集合竞价 (14:57-15:00)
        现与深交所一致，14:57 后不再是连续竞价
    ...
    Examples:
        ...
        False
        >>> is_continuous_trading_time(145700000)  # 14:57 收盘集合竞价开始
        False
...
def get_trading_session(tick_time: int) -> str:
    ...
    Returns:
        str: 时段名称
            ...
            - 'afternoon_continuous': 下午连续竞价 (13:00-14:57)
...
if __name__ == '__main__':
    # 验收测试用例 (2018年8月后规则: 14:57 后为收盘集合竞价)
    test_cases = [
        (93000000, True, "9:30 连续竞价开始"),
        (92500000, False, "9:25 开盘集合竞价"),
        (145659999, True, "14:56:59.999 下午连续竞价最后一刻"),
        (145700000, False, "14:57 收盘集合竞价开始 (2018年8月后)"),
        (145959999, False, "14:59:59.999 收盘集合竞价中"),
